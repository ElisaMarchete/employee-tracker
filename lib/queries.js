...
// function to validate if the input length is greater than 0
...
// function to call all the functions below
function processAnswer(answer) {
  ...
  // call the functions to view, add, update, and delete data from the database
  viewTables(answer);
  ...
  // clear the input data to better visibility when inserting data
  if (
...
// First input displayed to the user
function userInput() {
  inquirer
    .prompt([
      {
    ...
    .then((answer) => {
      // Fucntion which will call all the functions below
      processAnswer(answer);
    });
}

// Function to view all the tables
...
function addRole(answer) {
  if (answer.options === "Add Role") {
    connection.query("SELECT * FROM department", function (err, rows) {
      ...
      // new pronpt to capture the role information
      inquirer
        .prompt([
          {
            type: "input",
        ...
        .then((response) => {
          ...
          // select the department id from the department table to use the result in the role table
          connection.query(
...
// Function to see the total utilized budget by department
          ...
          connection.query(
            ...
            function (err, results) {
              if (results === undefined) {
                console.log("\n");
                console.log("Unable to delete. In use by another table.");
              } else {
                console.log("\n");
...
// export the userInput function to be used in index.js
module.exports = {
  userInput,
};
